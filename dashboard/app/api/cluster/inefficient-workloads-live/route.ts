...
function calculateInefficiency(params: {
  ...
  if (!params.hasCpuRequest) {
    inefficiencyScore += 30;
    issues.push("Missing CPU request");
  }

  if (!params.hasMemoryRequest) {
    inefficiencyScore += 25;
    issues.push("Missing memory request");
  }

  if (!params.hasCpuLimit) {
    inefficiencyScore += 10;
    issues.push("Missing CPU limit");
  }

  if (!params.hasMemoryLimit) {
    inefficiencyScore += 10;
    issues.push("Missing memory limit");
  }
  ...
  if (params.hasCpuRequest && params.cpuRequestMillicores > 0) {
    ...
    if (usageRatio < 0.1) {
      inefficiencyScore += 25;
      issues.push("CPU request appears highly overprovisioned");
    } else if (usageRatio < 0.25) {
      inefficiencyScore += 15;
      issues.push("CPU request appears oversized");
    }
  }

  return {
    inefficiencyScore: Math.min(100, inefficiencyScore),
    issues,
  };
}
  ...
  try {
    ...
    const workloads =
      parsedDeployments.items?.map((item: K8sDeployment) => {
        ...
        const result = calculateInefficiency({
    ...
    const sortedWorkloads = [...workloads].sort(
      (a, b) => b.inefficiencyScore - a.inefficiencyScore
    );

    return NextResponse.json({
      success: true,
      workloads: sortedWorkloads,
      topWorkloads: sortedWorkloads.slice(0, 5),
    });
  } catch (error: any) {
    return NextResponse.json(
      {
        ...
        error:
          error?.message ??
          "Unknown error while calculating inefficient workloads",
      },
      { status: 500 }
    );
  }
